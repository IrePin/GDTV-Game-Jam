...
namespace _DrRush.Scripts.Runtime.Components
{
	/// <summary>
        ...
        private void MakeBridgeToMini()
        {
	        ...
	        ContextLocator.Instance.OnAddItemCompleted.AddListener(context =>
	        {
		        ...
		        context.ModelLocator.OnAddItemCompleted.AddListener(model =>
		        {
			        ...
			        DrRushModel drRushModel = (DrRushModel)model;
			        if (drRushModel != null)
			        {
				        drRushModel.IsGameOver.OnValueChanged.AddListener(IsGameOver_OnValueChanged);
				        drRushModel.IsGamePaused.OnValueChanged.AddListener(IsGamePaused_OnValueChanged);
			        }
		        });
	        });
        }
